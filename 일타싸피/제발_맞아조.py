...
NICKNAME = 'GWANGJU03_PLEASE_STRIKE'
...
while True:
    ...
    if order == 1:
        ...
        # 목적구와 홀의 거리가 멀다면 거리 비례 강하게 공을 친다
        power = 85
        # 목적구와 홀의 거리가 가깝다면 거리 비례 비교적 약하게 공을 친다
        # if targetBall_x < 10 or targetBall_y < 10:
        #     power = distance * 0.4
        # elif 117 < targetBall_x < 137 or 117 < targetBall_y < 137:
        #     power = distance * 0.4
        # elif targetBall_x > 244 or targetBall_y > 244:
        #     power = distance * 0.4
        ...
        # tx, ty : 3번의 X, Y 좌표를 나타내기 위한 변수
        # tx = balls[3][0]
        # ty = balls[3][1]
        # 흰 공이 5번 홀 근처에 있고, 3번공이 3번 홀 근처에 있다면 +250도
        # if abs(HOLES[5][0] - whiteBall_x) < 10 and abs(HOLES[5][1] - whiteBall_y) < 10 and abs(HOLES[3][0] - tx) < 10 or abs(HOLES[3][1] - ty) < 10:
